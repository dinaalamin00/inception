...
# 1️⃣ Initialize MariaDB data directory if empty
if [ ! -d "/var/lib/mysql/mysql" ]; then
    echo "Initializing MariaDB data directory..."
    mysqld --initialize-insecure --user=mysql
fi

# 2️⃣ Start MariaDB temporarily in background
echo "Starting temporary MariaDB server..."
mysqld_safe --skip-networking &
pid="$!"
# --skip-networking ensures no external connections during init
sleep 5  # wait for the server to start

# 3️⃣ Set root password if provided
if [ -n "$MYSQL_ROOT_PASSWORD" ]; then
    echo "Setting up root user..."
    mysql -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD';"
fi

# 4️⃣ Optional database
if [ -n "$MYSQL_DATABASE" ]; then
    echo "Creating database $MYSQL_DATABASE..."
    mysql -e "CREATE DATABASE IF NOT EXISTS $MYSQL_DATABASE;"
fi

# 5️⃣ Optional non-root user
if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_PASSWORD" ]; then
    echo "Creating user $MYSQL_USER..."
    mysql -e "CREATE USER IF NOT EXISTS '$MYSQL_USER'@'%' IDENTIFIED BY '$MYSQL_PASSWORD';"
    mysql -e "GRANT ALL PRIVILEGES ON $MYSQL_DATABASE.* TO '$MYSQL_USER'@'%';"
...
# 6️⃣ Run all initialization scripts
for f in /docker-entrypoint-initdb.d/*; do
    case "$f" in
        *.sh)  echo "Running $f"; . "$f" ;;
        *.sql) echo "Running $f"; mysql < "$f" ;;
...
# 7️⃣ Stop temporary server
echo "Stopping temporary MariaDB server..."
mysqladmin shutdown

# 8️⃣ Run MariaDB in foreground to keep container alive
